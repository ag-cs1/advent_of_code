...
def check_column(i, board):
    for j in range(0, len(board.grid[i])):
        if not board.grid[i][j].marked:
            return False
    return True
...
def main():
    ...
    num, winning_board = play_bingo(drawn_nums, boards)
    print(winning_board.total * int(num))
